...
Fusion: Reciprocal Rank Fusion (k=60) → top-80 кандидатов

Reranking: Cross-encoder (bge-reranker-v2-m3) → top-12 финальных чанков

Графовое расширение: 1-hop по связям документов → +2-4 контекстных чанка
...
✨ Русская лемматизация — критическое улучшение для 70% контента
✨ Экстрактивный QA fallback — надежность +40%, независимость от LLM API
✨ Circuit breaker — устойчивость к отказам провайдеров
✨ Семантический кеш — снижение LLM-запросов на 30-40%
✨ Zram вместо swap — в 2-3 раза быстрее при memory pressure
✨ Thin wrapper — экономия ~200 MB RAM, контроль архитектуры
...
Первые шаги (сегодня)
Форкнуть репозиторий структуры проекта
Настроить VPS + zram
Протестировать pymorphy3 на sample-данных из SiYuan
Создать первые 10 вопросов для золотого набора
...
Этот документ является живым артефактом. Все изменения архитектуры, метрик и рисков должны отражаться здесь с обоснованием.
